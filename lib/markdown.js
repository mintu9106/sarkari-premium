...
export function renderMarkdown(md) {
  ...
  // 4b. Links conversion
  html = html.replace(/\[([^\]]+)\]\((https?:\/\/[^\s)]+)\)/g, '<a href="$2" target="_blank" rel="noopener noreferrer" class="text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 underline font-semibold transition-colors">$1</a>');

  // 5. Unordered lists conversion
  // Match single bullet points (dashes or asterisks) and wrap them in <li> tags
  html = html.replace(/^\s*[-*]\s+(.*$)/gim, '<li>$1</li>');

  // 5b. Ordered lists conversion
  // Match single numbered points (e.g. "1. Visit...") and wrap them in <li class="ordered-item"> tags
  ...
  // Assemble back and group adjacent lists inside <ul> / <ol>
  let finalHtml = resultLines.join('\n');
  
  // Replace sequential <li class="ordered-item"> tags with <ol class="markdown-ordered-list">...</ol>
  finalHtml = finalHtml.replace(/(<li class="ordered-item">.*?<\/li>(\s*\n\s*<li class="ordered-item">.*?<\/li>)*)/gs, '<ol class="markdown-ordered-list">$1</ol>');
